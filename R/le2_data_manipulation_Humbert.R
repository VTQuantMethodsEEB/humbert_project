...
fs$lgdL = log10(fs$ASPECT)

# Summarise
fs %>% 
  group_by(ASPECT) %>% 
  summarise(mean.aspect=mean(lgdL,na.rm=TRUE))
...
lfi.table = fs %>% 
  group_by(LFI) %>% 
  summarise(mean.lfi=mean(lgdL,na.rm=TRUE))
...
fs<-aggregate(TYPE~SEX+ASPECT+LFI,data=fs, FUN=mean)  #make a df of bat counts
